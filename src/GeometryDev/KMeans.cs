namespace GeometryDev
{
    public class KMeans
    {
        // ref: https://visualstudiomagazine.com/articles/2013/12/01/k-means-data-clustering-using-c.aspx
        public static Tuple<int,PointDto>[] Cluster(PointDto[] points, int numClusters)
        {
            // k-means clustering
            ...
            double[][] rawData = new double[points.Length][];
            for(var i = 0; i< points.Length; i++)
            {
                rawData[i] = new double[] { points[i].Lng, points[i].Lat };
            }
            ...
            var result = new Tuple<int, PointDto>[points.Length];
            for(var i = 0; i< points.Length; i++ )
            {
                result[i] = new Tuple<int, PointDto>(clustering[i], points[i]);
            }
            return result;
        }
